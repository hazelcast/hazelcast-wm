...
- `notify-cluster`: If set to true, whenever a session is used the cluster will be notified to reset the idle-time of this session on the distributed map. The cluster map is already notified for all modification operations but this might be required when consecutive read-attribute operations are performed without any modification. Since these attributes will be fetched from the local cache, the session on the distributed map might be evicted even if the session is active. Note that this is useless when non-sticky sessions are used or max-idle-second is not set for the cluster map. It's default value is false.
